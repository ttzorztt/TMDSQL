 ...
 * @LastEditTime : 2023-01-13 17:02:56
 */
#ifndef _LOG_
#define _LOG_
#ifndef _FILE_H_
...
class Log {
 ...
 public:
   ...
   * @return
   */
  void static LogForExit(std::string& UserName, TYPE_POWER& op,TYPE_ERROR_CASE errorCase = 顺利执行);
  /**
   * @brief 登录
   * @param  string& UserName 用户名
   * @param  TYPE_POWER& op 权限
   ...
   * @return
   */
  void static LogForCreateTable(std::string& UserName,
                                TYPE_POWER& op,
                                std::string& DBID,
                                std::string& TBID,
                                TYPE_ERROR_CASE errorCase = 顺利执行);
