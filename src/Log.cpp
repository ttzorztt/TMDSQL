...
void Log::LogForError(std::string UserName, TYPE_POWER op, vCID command,
                      revstring data, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  std::string tmp;
  if (command.size() != 0) {
    for (auto& CI : command) {
...
std::string Log::nowDataTime() {
  ...
  _file backfile(FileName, type::_TYPE_BACK);
  if (!backfile.isExist()) {
    backfile.create();
		backfile.write("0",type_mode::WRITEBUFF_MODE_APP);
  }
...
void Log::LogForSelectDatabase(std::string UserName, TYPE_POWER op,
                               std::string DBID, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "}" + " => 选择 数据库\"" + DBID + "\"",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForExecuteSQL(std::string UserName, TYPE_POWER op,
                           std::string SQLPath, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "}" + " => 执行 \"" + SQLPath + "\"",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForSelectDatabaseTable(std::string UserName, TYPE_POWER op,
...
void Log::LogForShow(std::string UserName, TYPE_POWER op,
                     TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 显示",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForShowAllDatabase(std::string UserName, TYPE_POWER op,
                                TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 显示 数据库",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForShowDatabase(std::string UserName, TYPE_POWER op,
                             std::string DBID, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 显示 数据库\"" + DBID + "\"",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForShowDatabaseTable(std::string UserName, TYPE_POWER op,
                                  std::string DBID, std::string TBID,
                                  TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
...
void Log::LogForExit(std::string UserName, TYPE_POWER op,
                     TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 退出",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForLogin(std::string UserName, TYPE_POWER op, std::string LoginID,
                      std::string LoginPassword, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 登录 用户\"" + LoginID +
                 "\" 密码\"" + LoginPassword + "\"",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForCreateUser(std::string UserName, TYPE_POWER op, std::string ID,
                           std::string Pd, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 创建 普通用户\"" + ID +
                 "\" 密码\"" + Pd + "\"",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForCreateManager(std::string UserName, TYPE_POWER op,
...
void Log::LogForCreateDatabase(std::string UserName, TYPE_POWER op,
                               std::string DBID, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 创建 数据库\"" + DBID + "\"",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForCreateDatabaseTable(std::string UserName, TYPE_POWER op,
                                    std::string DBID, std::string TBID,
                                    TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
...
void Log::LogForDeleteDatabase(std::string UserName, TYPE_POWER op,
                               std::string DBID, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 删除 数据库\"" + DBID + "\"",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForDeleteDatabaseTable(std::string UserName, TYPE_POWER op,
                                    std::string DBID, std::string TBID,
                                    TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
...
void Log::LogForDeleteUser(std::string UserName, TYPE_POWER op, std::string UID,
                           TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 删除 普通用户\"" + UID + "\"",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForDeleteManager(std::string UserName, TYPE_POWER op,
                              std::string MID, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
  file.write(TP[op] + "{" + UserName + "} => 删除 管理员\"" + MID + "\"",
             type_mode::WRITEBUFF_MODE_APP);
}
void Log::LogForInsertDatabaseTable(std::string UserName, TYPE_POWER op,
                                    std::string DBID, std::string TBID,
                                    revstring data, TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
...
void Log::LogForFindDatabaseTable(std::string UserName, TYPE_POWER op,
                                  std::string DBID, std::string TBID,
                                  std::string Index,
                                  TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
...
void Log::LogForSetIndexDatabaseTable(std::string UserName, TYPE_POWER op,
                                      std::string DBID, std::string TBID,
                                      std::string index,
                                      TYPE_ERROR_CASE errorCase) {
  inputNowTime(errorCase);
...
void Log::LogForSetViewDatabaseTable(std::string UserName, TYPE_POWER op,
                                     std::string DBID, std::string TBID,
