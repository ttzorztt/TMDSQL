...
- V(1.3)
    > 差不多版本,目标实现多用户同时登录,对表写锁,不允许相同用户登录多次。不会真的会有一个用户无聊到同时去登录两个终端然后测试健壮性吧？有多少代码是给这些无聊的用户写的。
...
- 内核加锁层
	> 由于想实现多个用户同时登录,那么就会有可能两个脑残用户同时操作一个表,或会影分身的用户在两个终端同时登录,抵制超能力。同时两个脑残用户同时操作,我们也需要对日志进行加锁保护。
- 前端shell解释层
...
- 锁子层(实在是组词太难了)
  - 对于某些有点小毛病的用户开两个终端,哎非要登录同一个帐号,还非要操作同一个表,但我还不能骂他,只能悄悄的进村,打枪的不要,对登录的用户上锁,防止某个有意泄漏密码的吗卡巴卡的让另外一个小豆豆同时登录。我还得预防两个吗卡巴卡登录的时候,哎,非要同时写入一个表,表被写乱又怨代码不行,真的是,就是那种上不了炕怨炕不平的人。那对写入,删除文件,删除一行等对表修改的操作加锁,那万一有另外一个小豆豆正在读文件读到一半,忽然发现表被删了,要跳楼那怎么办？其实觉得把,百分之九十的代码都是多余的。就是为了防止这些玛卡巴卡误操作。
- 脚本层
...
- 视图层
    > 在./data/view文件夹中，同样与database分布相同，每行保存数据为: \[用户名\] allowCol1 allowCol2 ...,就是记录小趴菜允许访问的列。
