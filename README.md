...
- V(1.0)
  > 初始版本，实现对数据库和表的简要增删查，和权限管理，前端自定义中文shell脚本编程。
- V(1.1)
  > 对上一个版本的BUG进行修复，同时新增日志管理，更为详细记录shell的报错信息。
  - 刚测试发现一个BUG，在windows版本下，日志记录有延迟，执行当前指令，日志中显示的当前指令并不全，但当系统正常退出时，日志中就会把缺陷的部分补齐。而在Linux版本中，就没有这个这个BUG。考虑应该是内核的原因。
- V(1.2)
  > 对上一个版本的BUG进行修复，目标实现用户视图层
...
- 权限管理
  ...
  - 管理员
  - 超级管理员Root
  > 不同的权限可以执行不同的操作。上级用户可以对下级用户进行增删，也就是说Root可以增删管理员和用户，管理员可增删用户，普通用户只能查询，增删需要管理员以及ROOT才可执行。
- 前端shell编写
  - 实现简单的解释，Shell是按照解释型语言编写，大概是27条中文指令，分为9类。不同指令执行会在终端输出是否成功以及简要错误原因。同时会在日志中记录更为详细的原因。
    - 退出系
      > 退出系统
    - 选择系
      > 如果选择了数据库，后期建表或删表等操作的指令可以省略数据库参数，同样如果选择了表，后期对表的指令同样可以省略表对应的参数
    - 显示系
      > 对数据库所存储的表，或表的内容进行显示
    - 登录系
      > 系统权限划分很严格，不同权限下允许执行的指令不同
    - 执行系
      > 主要为了实现脚本化编程，另外建立一个txt文件，包含若干指令，使用执行系指令可以直接执行脚本
    - 创建系
      > 对数据库，表，用户，管理员的创建
    - 删除系
      > 对数据库，表，用户，管理员的删除
...
- 脚本系统
  > 支持脚本编程，即不需要用户在终端中一条一条执行，将指令写在一个文件中，每行存储一条指令，主要中间不允许空行(会认为文件结束)。把该文件放在./data/SQL/路径下，使用指令 `执行 @SQLName`执行。
- PCB记录
  > 所有表项均有对应PCB文件，存放在./data/PCB/中，该文件夹和database分布相同。记录一些需要的表属性。
...
### UserName
  ...
  > 只有一个文件，存放帐号密码信息。当然是明码了，你个锁黑，没事搞什么加密算法干嘛。
...
- **显示**  **表**  _@TBID_  _showNumber_ （V1.1废弃)
- **显示**  **数据库**  **表**  _@DBID_ _TBID_ (V1.1废弃)
