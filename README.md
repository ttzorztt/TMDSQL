...
- V(1.3)
	...
	- bug记录： UTF-8中文是三个字节,GBK是两个字节,假设你在终端中输入多个汉字,按下退格键后,只会删除一个字节(或许是一个),但问题是OS似乎只允许你对一个汉字退格一次。导致你多次退格后留有汉字删不掉。
...
> 似乎要解决这个问题,我似乎将整个IO从中间打断了。截取每一个字符,截取backspace和回车,打乱英文默认的模式。起始输入一个中文汉字,从编码UTF-8来说是相当于输入连续三个字符。只不过是大于0x80的字符。相当于打断IO重新用buff建立了一个中文习惯的IO.
