...
- Cache中存储5个文件
- 使用先进先出的替换策略
- 触发条件是在TMDSQL中进行查询操作,首次查询会走文件查询,硬暴力走索引文件获取文件指针。查询结束后会将该文件对应的索引文件放入Cache中,用map将索引与文件指针连接起来。使得达到o(1)的速度。
但是还是有一些缺陷,就是替换策略有点简单,以及文件大小没有做限制。特别大的数据的索引可能会爆内存。
